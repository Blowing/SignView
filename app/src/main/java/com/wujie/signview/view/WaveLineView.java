...
//            for (int i = 0; i <= samplingSize; i++) {
//                float x = samplingX[i];
//                curY = (float) (amplitude * calcValue(mapX[0], offset));
////                paths.get(0).lineTo(x, centerHeight);
////                paths.get(1).lineTo(x+20,centerHeight);
////                paths.get(2).lineTo(x+40,centerHeight);
//                for (int n = 0; n < paths.size(); n++) {
//
//                    //四条线分别乘以不同的函数系数
//                    float realY = curY * pathFuncs[n] * volume * 0.01f;
...
//            }
            ...
            for (int i = 0; i < paths.size(); i++) {
                ...
                paths.get(i).lineTo(width,height);
            ...
            canvas.drawRect(new Rect(0,centerHeight, width, height), paint);
    ...
    private double calcValue(float mapX, float offset) {
        ...
        offset %= 2;
        double sinFunc = Math.cos(Math.PI * mapX - offset * Math.PI);
